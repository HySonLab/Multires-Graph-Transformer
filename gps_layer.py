...
class GPSLayer(nn.Module):
    ...
    def __init__(self, dim_h,
        ...
        # MPNNs without edge attributes support.
            ...
            self.local_model = pyg_nn.GCNConv(dim_h, dim_h)
        elif local_gnn_type == 'GIN':
            self.local_gnn_with_edge_attr = False
            gin_nn = nn.Sequential(Linear_pyg(dim_h, dim_h),
                                   self.activation(),
                                   Linear_pyg(dim_h, dim_h))
            self.local_model = pyg_nn.GINConv(gin_nn)

        # MPNNs supporting also edge attributes.
        elif local_gnn_type == 'GENConv':
            self.local_model = pyg_nn.GENConv(dim_h, dim_h)
        elif local_gnn_type == 'GINE':
            gin_nn = nn.Sequential(Linear_pyg(dim_h, dim_h),
                                   self.activation(),
                                   Linear_pyg(dim_h, dim_h))
           
            self.local_model = pyg_nn.GINEConv(gin_nn)
        elif local_gnn_type == 'GAT':
            self.local_model = pyg_nn.GATConv(in_channels=dim_h,
            ...
            self.local_model = pyg_nn.PNAConv(dim_h, dim_h,
        ...
        if self.layer_norm:
            ...
            # self.norm1_local = pyg_nn.norm.GraphNorm(dim_h)
            # self.norm1_attn = pyg_nn.norm.GraphNorm(dim_h)
            # self.norm1_local = pyg_nn.norm.InstanceNorm(dim_h)
            # self.norm1_attn = pyg_nn.norm.InstanceNorm(dim_h)
        if self.batch_norm:
        ...
        if self.layer_norm:
            ...
            # self.norm2 = pyg_nn.norm.GraphNorm(dim_h)
            # self.norm2 = pyg_nn.norm.InstanceNorm(dim_h)
        if self.batch_norm:
    ...
    def forward(self, batch):
        ...
        if self.local_model is not None:
            self.local_model: pyg_nn.conv.MessagePassing  # Typing hint.
            if self.local_gnn_type == 'CustomGatedGCN':
                ...
                x, edge_attr = self.local_model(h, batch.edge_index, batch.edge_attr, batch.pos)
